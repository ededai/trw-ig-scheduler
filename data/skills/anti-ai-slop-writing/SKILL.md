...
**Em dash (—):** Maximum ONE per 500 words, and only if truly necessary. The single most cited AI tell. Strongly prefer commas, semicolons, colons, or parentheses. When in doubt, rewrite instead.
**Double dash (--):** NEVER. Not once. Not "just this one time." Always rewrite.
...
6. Any double dash (--)? → Remove, rewrite. Any em dash (—)? → Count them. More than one per 500 words, or used out of laziness rather than necessity? → Rewrite.
